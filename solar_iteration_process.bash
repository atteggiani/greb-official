...
tsurf=${tsurf:="${wdir}/output/scenario.exp-931.geoeng.sw.artificial.frominput_x0.98_50yrs"}
tsurf_init=$tsurf
...
while (( $niter <= $tot_iter )); do
    ...
    ./myjobscript.bash -y ${sim_years%yrs} -s "${wdir}/artificial_solar_radiation/sw.artificial.iteration${ocean_flag}/sw.artificial.iter${niter}${ocean_flag}"
    # Change files for next iteration
    tsurf="${wdir}/output/scenario.exp-931.geoeng.sw.artificial.iter${niter}${ocean_flag}_${sim_years}"
    ((niter++))
done
echo -e "\nPlotting iterations...\n"
python ./plot_iter.py $tsurf_init 'solar' $ocean_flag
echo -e "DONE!!\n"
exit
