...
sw_files=../artificial_solar_radiation/r_calibration/*.bin
nfiles=$(ls -1 $sw_files | wc -l)
count=0
for sw_file in $sw_files ; do
    rm swart
    ((count++))
    echo ""
    echo "Iter ${count}/${nfiles}: $sw_file"
    ln -s $sw_file swart
    # get name for output file
    name=$(basename ${sw_file%.*})
    ...
    mv scenario.bin ../output/r_calibration_solar/scenario.${FILENAME}.bin
    cat >../output/r_calibration_solar/scenario.${FILENAME}.ctl <<EOF
...
EOF
    ...
    then
        mv scenario.gmean.bin ../output/r_calibration_solar/scenario.gmean.${FILENAME}.bin
        cat >../output/r_calibration_solar/scenario.gmean.${FILENAME}.ctl <<EOF
...
EOF
    ...
    # rename control run output and move it to output folder
        mv control.bin ../output/r_calibration_solar/control.${FILENAME}.bin
        cat >../output/r_calibration_solar/control.${FILENAME}.ctl <<EOF
