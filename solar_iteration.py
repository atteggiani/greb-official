...
r_sw=from_binary(constants.greb_folder()+'/r_calibration_solar').r
corr = 0.2

t_init_fname = constants.output_folder()+'/scenario.exp-931.geoeng.sw.artificial.frominput_x0.98_50yrs'
...
folder = constants.solar_folder()+'/sw.artificial.iteration{}'.format(ocean_flag)
os.makedirs(folder,exist_ok=True)

if niter == 1:
    sw_fname = constants.get_art_forcing_filename(t_fname,'solar')
else:
    sw_fname = constants.get_art_forcing_filename(t_fname,'solar',output_path=folder)
...
if ocean_flag is "_ocean":
    new_solar = np.where(constants.land_ocean_mask(),dsw_new+sw_base,sw_base)
else:
